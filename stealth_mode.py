...
def extract_text_from_pdf(pdf_path):
    ...
    try:
        ...
        st.info(f"pdfplumber extraction failed: {e}")
    ...
    try:
        ...
        st.error(f"PyPDF2 extraction also failed: {e}")
...
def analyze_and_update_resume(resume_text: str, job_description_text: str):
    ...
    try:
        ...
        # Create prompt template with stronger emphasis on improvement
        prompt = ChatPromptTemplate.from_messages([
            ("system", """You are an expert ATS scorer and resume optimization assistant.
        ...
        # Ensure initial score is reasonable (not too low to be discouraging)
        initial_score = max(30, initial_analysis.initial_match_score)  # Minimum score of 30
        ...
        # Score the updated resume with emphasis on improvement
        with st.spinner("Calculating improved score..."):
            scoring_prompt = ChatPromptTemplate.from_messages([
                ("system", """You are an expert ATS scorer. 
                Score this IMPROVED resume against the job description.
                The resume has been optimized with better keywords and phrasing.
                
                IMPORTANT: This is an IMPROVED version of a resume that previously scored {initial_score}/100.
                The improvements include better keyword matching, clearer experience descriptions, and better alignment with job requirements.
                The score should reflect these improvements and be HIGHER than {initial_score}.
                
                Provide only the integer score out of 100."""),
                ("human", """Score this IMPROVED resume against the job description:
            ...
            try:
                ...
                if match:
                    updated_score = int(match.group(0))
                    # Ensure the updated score is higher than initial score
                    updated_score = max(updated_score, initial_score + 10)
                    updated_score = min(100, updated_score)  # Cap at 100
                else:
                    # If parsing fails, ensure we show improvement
                    updated_score = min(initial_score + 15, 95)
            except:
                # Fallback: show reasonable improvement
                updated_score = min(initial_score + 15, 95)
        ...
        result = {
            ...
            "suggested_updated_resume_content_json": [sec.model_dump() for sec in initial_analysis.updated_resume_content_suggestion],
            "updated_score": updated_score,
            "score_comparison": f"Initial score: {initial_score}, Updated score: {updated_score}"
        }
...
with col2:
    ...
    if input_method == "Upload PDF":
        ...
        job_description_text = st.text_area("Paste job description here", height=200, placeholder="Paste the job description text here...")
        if job_description_text:
            st.success(f"✅ Job description provided")
...
if st.button("🚀 Analyze Resume", type="primary", use_container_width=True):
    ...
    if not api_key:
        ...
        try:
            ...
            if not resume_text:
                st.error("Could not extract text from the resume PDF. Please ensure it contains selectable text.")
            else:
                # Extract job description based on input method
                if input_method == "Upload PDF":
                    with tempfile.NamedTemporaryFile(delete=False, suffix='.pdf') as tmp_job:
                        tmp_job.write(job_file.read())
                        tmp_job.flush()
                    ...
                    with st.spinner("Fetching job description from URL..."):
                        try:
                            ...
                            job_description_text = None
                # For "Paste Text", job_description_text is already set
                ...
                if job_description_text:
                    ...
                    st.error("Could not extract or fetch the job description. Please try a different input method.")
...
if st.session_state.analysis_complete and st.session_state.analysis_result:
    ...
    initial_score = result['initial_score']
    updated_score = result['updated_score']
    score_diff = updated_score - initial_score
    ...
    with col3:
        st.markdown('<div class="score-card score-card-improvement">', unsafe_allow_html=True)
        improvement_percentage = (score_diff / initial_score * 100) if initial_score > 0 else 0
        st.markdown(f'''
        <div style="margin-top: 20px;">
            <div class="score-value">{improvement_percentage:.1f}%</div>
            <div class="score-label">Total Improvement</div>
